...
public class HiveMind
{
	...
	public Boolean saveState()
	{
		...
		String hiveModelFilename=getHiveModelFilename(product, filePath);
		if(hiveModelFilename!=null)
		{
			if(loadedModelFilename!=null && !loadedModelFilename.equals(hiveModelFilename))
			{
				try
				{
					// Another model has been saved before the end of the session - merge models to avoid lost information
					...
					if(object!=null)
					{
						...
						{
							// Use the other model and merge with the changes performed during the session
							AppState previousStateTree=(AppState)previousObject;
							AppState currentStateTree=stateTree;
							stateTree=(AppState)object;
							mergeStates(previousStateTree, currentStateTree, stateTree);
						}
					}
				}
				catch(Exception e)
	...
	private String getHiveModelFilename(String product, String filePath)
	{
		List<File> files=getFolderFiles(filePath);
    String brainFilename=getLastModelFilename(files);
	...
	/**
	 * Get the model with the highest name (latest)
	 * @param files
